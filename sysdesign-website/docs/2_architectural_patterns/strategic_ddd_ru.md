...
Стратегическое предметно-ориентированное проектирование определяет фундаментальные компоненты для понимания и моделирования сложных бизнес-областей: **домен**, **поддомены**, **ограниченные контексты**, **карты контекстов** и **паттерны интеграции**. Эти концепции связывают реальность бизнеса с архитектурой программного обеспечения, обеспечивая соответствие систем реальным бизнес-процессам, а не предполагаемым шаблонам организации программного обеспечения.

В корпоративных системах часто возникают сложности с определением границ: определением независимости сервисов, группировкой функций и критериями разделения систем. Стратегический DDD предоставляет систематический подход к принятию этих решений, ставя во главу угла понимание бизнес-области, а не технические вопросы.

## Технологические ограничения границ

Некоторые команды начинают с разделения систем по техническим слоям: API-шлюзы, сервисы бизнес-логики, слои доступа к данным, сервисы аутентификации. Этот подход может показаться естественным для разработчиков, но часто упускает из виду сложность бизнеса и приводит к созданию распределенных монолитных систем, которые обладают всей сложностью микросервисов, но не имеют никаких преимуществ.
...
```mermaid
graph TD
    A[Веб API<br/>шлюз] --> B[Сервис бизнес-логики<br/>]
...
- Знания о предметной области распыляются
- Изменения требуют координации между командами

Стратегический DDD предлагает другой подход: начните с понимания вашей бизнес-области, а технические решения будут естественным образом вытекать из границ бизнеса.

## Стратегический дизайн: картографирование территории бизнеса
...
**Домен** представляет собой общую область вашего бизнеса — полную сферу знаний и деятельности, в которой работает ваша организация. Внутри этого домена существуют **поддомены**, которые представляют собой отдельные области функциональности, которые можно понять и развить в некоторой степени независимо друг от друга.

#### Идентификация поддоменов

Начните с картирования возможностей, а не технических систем. Спросите экспертов предметной области: «Чем занимается ваша компания?»
...
- **Ищите специализированные знания**: области, требующие экспертов предметной области, обычно представляют собой отдельные поддомены
...
- **Определите границы принятия решений**: где применяются разные правила и политики
...
**Event Storming (мозговой штурм событий)**: совместный семинар, на котором эксперты предметной области и разработчики отображают все события домена (то, что происходит в бизнесе) на временной шкале. Это позволяет выявить бизнес-процессы, определить границы ограниченного контекста с помощью кластеризации событий и обнаружить концепции домена, которые могут быть упущены при традиционном сборе требований.

**Domain Storytelling**: метод, при котором эксперты предметной области рассказывают истории о том, как выполняется работа, а участники создают наглядные представления этих историй. Это помогает идентифицировать участников, объекты работы и действия, делая явными неявные знания о предметной области и выявляя естественные границы процессов.

Эти методы подчеркивают важность сотрудничества с экспертами предметной области и сосредоточены на понимании реальности бизнеса перед принятием технических решений.
...
Каждый ограниченный контекст имеет свою собственную модель того, что такое клиент, оптимизированную для задач этого контекста. Мы можем создать различные артефакты для работы с ними (например, классы в Java или сервисы). Это не злонамеренное дублирование — скорее признание того, что одно и то же бизнес-понятие служит разным целям в разных контекстах.
...
Два контекста, разработанные командами, которые тесно сотрудничают. Изменения координируются, и обе команды имеют равное влияние на отношения. Используется, когда контексты тесно связаны по функционалу.
...
**Предупреждающие признаки**: каждое изменение требует обновления нескольких контекстов, большое количество синхронной коммуникации между контекстами, общие базы данных для всех контекстов.
...
Стратегический DDD предоставляет систематический подход к определению границ системы на основе бизнес-реальности, а не технического удобства. Начав с понимания вашей бизнес-области и используя стратегические паттерны — домен, поддомены, ограниченные контексты, карты контекстов и паттерны интеграции — вы можете создать системы, которые отражают реальную работу вашей организации.

Это итеративный процесс — ваши границы будут развиваться по мере роста вашего бизнеса и понимания. Начните с понимания домена, определите ограниченные контексты с помощью лингвистических границ, сопоставьте отношения и проверьте их на реальных бизнес-сценариях.

Помните: цель не в том, чтобы с первого дня создать идеальные границы, а в том, чтобы создать лучшие границы, которые со временем будут улучшаться и эффективно поддерживать ваши бизнес-цели.